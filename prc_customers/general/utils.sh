...
set_numactl(){
	...
	if [[ $HLS_MODULE_ID ]]; then
		...
		MODULES=($(echo ${HABANA_VISIBLE_MODULES} | tr ',' ' '))
	else
		echo no modules specified, skip numactl
		return
	fi

	NODE_MEM=($($HL_TOPO | grep "^[$(IFS="|" ; echo "${MODULES[*]}")]" | awk '{print $4}' | uniq | tr '\n' ' '))
	NODE_CPUS=($($HL_TOPO | grep "^[$(IFS="|" ; echo "${MODULES[*]}")]" | awk '{print $2}' | uniq | sed 's/,//g' | tr '\n' ' '))

	if [ "${#NODE_MEM[@]}" -gt 1 ] || [ "${#NODE_CPUS[@]}" -gt 1 ];then
		echo "specified modules are not on the same numa node, skip numactl"
		return
	fi
	NUM_HPU_PER_NODE=$($HL_TOPO | grep "${NODE_CPUS}" | wc -l)

	CPUS_LOW=$(echo ${NODE_CPUS} | cut -d '-' -f 1)
	CPUS_UP=$(echo ${NODE_CPUS} | cut -d '-' -f 2)
	...
	CORES=$(IFS="," ; echo "${CORES[*]}")

	NUMA_CTL="numactl -C $CORES -m $NODE_MEM"
    echo "using '$NUMA_CTL' for module #.$(IFS=',' ; echo ${MODULES[*]})"
}
